...
export default function RocketGirlStory() {
  ...
  useEffect(() => {
    ...
    seedCards(words.map((w) => w.en), 'rocketgirl')
          ...
          <QuizTab
            ...
            onFinish={(correct, total, wrongEns) => {
              ...
              wrongEns.forEach((en) => {
                ...
                if (w) addWrongWord({ en: w.en, zh: w.zh, emoji: w.emoji ?? '🚀', from: story.title, module: 'rocketgirl' })
              })
            }}
            onPick={(en, correct) => {
              seedCards([en], 'rocketgirl')
              recordReview(en, correct, 'rocketgirl')
            }}
          />
        )}
      </SafeBoundary>
