...
function buildQuiz(storyWords: FGWord[]): QuizItem[] {
  const allPool = flyGuyStories.flatMap((s) => s.words)
...
export default function FlyGuyStory() {
  ...
  if (!story) {
    return (
      <div className="empty">
        <p>没有找到这个 Fly Guy 故事。</p>
        <Link to="/flyguy" className="btn">返回故事列表</Link>
      </div>
    )
  }
  ...
  return (
    <div className="page lesson-preview" style={moduleThemeVars(FG_THEME)}>
      <div className="page-head" style={mcStyle}>
        ...
        <div>
          <div className="page-kicker">Fly Guy · 第 {story.id} 关</div>
          <h1 className="page-title">{story.title}</h1>
        </div>
      </div>

      <SafeBoundary label="Fly Guy 学习">
        <div className="mode-badge mode-preview">🐝 Fly Guy 主题课</div>
...
function VocabTab({ words }: { words: FGWord[] }) {
...
function SentencesTab({ words }: { words: FGWord[] }) {
      ...
      {done ? (
        <div className="result-card">
          ...
          <div className="result-actions">
            ...
            <Link to="/flyguy" className="btn btn-soft">🐝 返回故事列表</Link>
          </div>
        </div>
      ) : (
        <>
